...
import GameOwnersDialog from './GameOwnersDialog';
...
const ClanGamesCard = () => {
    ...
    const [selectedGameForOwners, setSelectedGameForOwners] = useState(null);
    ...
    const fetchGamesData = useCallback(async () => {
        ...
        try {
            const { data: members, error: membersError } = await supabase
                .from('members')
                .select('codinome, avatar_url, profile_games, patente_atual')
                .is('data_saida', null)
    ...
    const handleOpenOwnersDialog = (game) => {
        setSelectedGameForOwners(game);
        setIsOwnersDialogOpen(true);
    };
    ...
    return (
        <>
                                    ...
                                    <Card className="bg-card/50 hover:bg-primary/10 transition-all duration-300 h-full flex flex-col group border border-transparent hover:border-primary/30">
                                        ...
                                        <CardContent className="flex-grow flex flex-col justify-between">
                                            <div>
                                                ...
                                                <Button variant="link" className="p-0 h-auto text-sm text-muted-foreground mb-3 flex items-center hover:text-primary" onClick={() => handleOpenOwnersDialog(game)}>
                                                    <Users className="mr-2 h-4 w-4" /> {game.owners.length} membros possuem este jogo.
                                                </Button>
                                            </div>
                                                ...
                                                <div className="flex flex-wrap gap-2 mt-auto">
                                                    {game.owners.slice(0, 7).map(owner => (
                                                        <div key={owner.codinome} className="group relative">
                                                            <img  
                                                                src={owner.avatar_url || `https://ui-avatars.com/api/?name=${owner.codinome}&background=1f2937&color=fff`}
                                                                alt={owner.codinome} 
                                                    ...
                                                    {game.owners.length > 7 && (
                                                         <button onClick={() => handleOpenOwnersDialog(game)} className="w-8 h-8 rounded-full bg-muted flex items-center justify-center text-xs font-bold hover:bg-primary/30 transition-colors">
                                                            +{game.owners.length - 7}
                                                        </button>
                                                    )}
                                                </div>
            ...
            <GameOwnersDialog
                isOpen={isOwnersDialogOpen}
                onOpenChange={setIsOwnersDialogOpen}
                game={selectedGameForOwners}
            />
        </>
    );
};
