...
import { CheckCircle, XCircle, Coins, Award, ArrowRight, Trophy, RotateCw } from 'lucide-react';
...
const QuestionFlow = ({ questions, memberData, setMemberData, onFinish }) => {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      if (earnedCoins > 0 || earnedPoints > 0) {
        const { error: updateError } = await supabase.rpc('increment_member_points_and_coins', {
            p_member_id: memberData.id,
            p_points_to_add: earnedPoints,
            p_coins_to_add: earnedCoins
        });
  ...
  const renderFinalScore = () => (
    ...
    >
      ...
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
        <Button onClick={onFinish} className="btn-secondary-dark text-lg py-3 px-8 w-full sm:w-auto">
          <RotateCw className="mr-2 h-5 w-5" /> Tentar Novamente
        </Button>
        <Button asChild className="btn-primary-dark text-lg py-3 px-8 w-full sm:w-auto">
