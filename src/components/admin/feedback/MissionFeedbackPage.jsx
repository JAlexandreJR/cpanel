...
import { Loader2, Users, Send, ThumbsUp, ThumbsDown, Meh, UserCheck, Award } from 'lucide-react';
...
const MissionFeedbackPage = () => {
  ...
  const fetchParticipantsAndFeedbacks = useCallback(async (missionId) => {
    if (!supabase || !missionId || !evaluatorMemberId) return;
    ...
    const { data: existingFeedbacks, error: feedbacksError } = await supabase
      ...
      .select('*')
      .eq('mission_id', missionId)
      .eq('evaluator_member_id', evaluatorMemberId);
    ...
    if (feedbacksError) {
      ...
      existingFeedbacks.forEach(fb => {
        initialFeedbacks[fb.evaluated_member_id] = {
    ...
    setIsLoadingParticipants(false);
  }, [supabase, toast, evaluatorMemberId]);

  useEffect(() => {
    if (selectedMissionId && evaluatorMemberId) {
      fetchParticipantsAndFeedbacks(selectedMissionId);
    } else {
      setParticipants([]);
      setFeedbacks({});
    }
  }, [selectedMissionId, fetchParticipantsAndFeedbacks, evaluatorMemberId]);
  ...
  const handleSubmitFeedbacks = async () => {
    ...
    try {
      if (feedbackPayloads.length > 0) {
        const { error: feedbackError } = await supabase.from('mission_feedback').upsert(feedbackPayloads, { 
          onConflict: 'mission_id,evaluated_member_id,evaluator_member_id' 
        });
        if (feedbackError) throw feedbackError;
      }
      
      toast({ title: "Feedbacks Salvos!", description: "Os feedbacks foram registrados com sucesso." });
      fetchParticipantsAndFeedbacks(selectedMissionId); // Refresh data
    } catch (error) {
    ...
    <motion.div initial="hidden" animate="visible" className="space-y-6">
      <Card className="glassmorphic">
        <CardHeader>
          ...
          <CardDescription>Selecione um evento ou treinamento para registrar o feedback dos participantes.</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingMissions || isLoadingEvaluator ? (
            ...
            <Select onValueChange={setSelectedMissionId} value={selectedMissionId} disabled={!evaluatorMemberId}>
              ...
              <SelectContent className="bg-card border-border">
                ...
                )) : <SelectItem value="no-missions" disabled>Nenhum evento disponível</SelectItem>}
              </SelectContent>
            </Select>
          )}
        </CardContent>
              ...
              {participants.length === 0 ? (
                 <Card className="glassmorphic">
                    <CardContent className="py-10 text-center">
                      ...
                      <p className="text-muted-foreground">Nenhum participante registrado para este evento ainda.</p>
                    </CardContent>
                  </Card>
              ) : (
                <>
                  {participants.map(p => {
                    if (p.members.id === evaluatorMemberId) {
                      return null; // Don't show form for self-evaluation
                    }
                    return (
                  ...
                  <div className="flex justify-end mt-6 gap-4">
                     <Button 
                        ...
                        title={!selectedMissionInfo?.coins_for_positive_feedback_bonus ? "Este evento não tem bônus configurado." : "Processar bônus de moedas para os participantes."}
                      >
