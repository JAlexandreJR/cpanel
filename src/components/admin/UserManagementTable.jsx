import React from 'react';
    ...
    const UserManagementTable = ({ users, onEdit, onDelete, isLoading, currentAuthUserId }) => {
      return (
        <div className="overflow-x-auto rounded-md border">
          <Table>
            <TableHeader className="bg-muted/30">
              <TableRow>
                <TableHead className="font-semibold">Nome / Codinome</TableHead>
              ...
              {users.map(userEntry => (
                <TableRow key={userEntry.id} className="hover:bg-muted/20 transition-colors">
                  ...
                  <TableCell className="py-3 text-right space-x-2">
                    ...
                    <Button variant="outline" size="sm" onClick={() => onDelete(userEntry)} className="hover:border-destructive hover:text-destructive" disabled={isLoading || userEntry.id === currentAuthUserId}>
