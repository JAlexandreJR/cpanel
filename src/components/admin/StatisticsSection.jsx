  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        supabase.from('justifications').select('end_date, status'),
      ]);
      ...
      const openJustificationsCount = justificationsData.filter(j => new Date(j.end_date) >= today && (j.status === 'APROVADO' || j.status === null)).length;
