    ...
    import { getPromotionDisplayInfo, YES_NO_OPTIONS_VALUES, PATENTE_ORDER_MAP } from '@/components/admin/members/utils';
    ...
    const CommandingOfficersPage = () => {
      ...
      useEffect(() => {
        ...
        const channel = supabase.channel('realtime-officers')
      ...
      const filteredMembers = useMemo(() => {
        const officerRankOrder = PATENTE_ORDER_MAP["Major"];
        return allMembers.filter(member => {
          const memberRankOrder = PATENTE_ORDER_MAP[member.patente_atual] || 0;
          if (memberRankOrder < officerRankOrder) {
            return false;
          }
        ...
        <motion.section variants={cardVariants} initial="hidden" animate="visible" className="space-y-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <h1 className="text-3xl font-semibold text-foreground flex items-center">
              <ShieldCheck className="mr-3 h-8 w-8 text-primary" /> Oficiais Comandantes
            </h1>
          </div>
          ...
          {isLoading ? (
            <div className="flex justify-center items-center h-64"><Loader2 className="h-12 w-12 text-primary animate-spin" /> <p className="ml-4 text-xl text-muted-foreground">Carregando oficiais...</p></div>
          ) : (
            <MembersTable 
    ...
    export default CommandingOfficersPage;
