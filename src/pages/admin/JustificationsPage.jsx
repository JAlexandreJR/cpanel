...
const JustificationsPage = () => {
  ...
  const renderTable = (data, title, isExpiredTab = false) => (
                ...
                {data.map(j => (
                  <TableRow key={j.id} className="hover:bg-muted/50">
                    ...
                    <TableCell className="text-center">
                      <Badge variant={isExpiredTab ? "secondary" : "default"}>
                        {isExpiredTab ? "VENCIDA" : (j.status || "ABERTO")}
                      </Badge>
                    </TableCell>
                  </TableRow>
                ))}
    ...
    <div className="p-1">
      <Tabs defaultValue="open" className="w-full">
        ...
        <TabsContent value="expired" className="mt-4">
          {renderTable(expiredJustifications, "Justificativas Vencidas", true)}
        </TabsContent>
      </Tabs>
    </div>
  );
